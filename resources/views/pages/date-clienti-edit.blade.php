...
<div class="bg-container bg-gray-30 h-full">
    <div class="mx-auto container py-3 my-6 font-body md:px-0 px-3">
        ...
        <h3 class="text-2xl font-bold py-4 text-gray-800 border-b">Date Client</h3>
        <form action="{{route('date-clienti.update', $client_id)}}" method="POST" enctype="multipart/form-data" class="mt-6">
            @csrf
            {{method_field('PUT')}}
            <div
            ...
            <div class="controls flex space-x-3">
                <a href="{{route('account.date-clienti')}}">
                    <div class="pt-6">
                        <div class="h-10 px-10 text-blue-700 border border-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            {{ __('Inapoi') }}
                        </div>
                    </div>
                </a>
